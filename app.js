...
function subleaseExpenses(c){
 ...
 let rows=arr.map(x=>{
   let buttons=x.type==='Аренда помещения'
    ? `<button class="btn secondary small" onclick="rentPaymentModal('${c.id}','${x.id}')">✎</button> <button class="btn danger small" onclick="delRentPayment('${c.id}','${x.id}')">×</button>`
    : `<button class="btn secondary small" onclick="expenseModal('sublease','${c.id}','${x.id}')">✎</button> <button class="btn danger small" onclick="delExpense('sublease','${c.id}','${x.id}')">×</button>`;
...
function projectModal(id){let x=id&&project(id);modal(x?'Редактировать проект':'Новый проект',`<div class="form-grid">${fld('name','Название проекта',x?.name||'')}</div>`,f=>{if(x)x.name=f.get('name');else state.projects.push({id:uid(),name:f.get('name'),children:[]})})}
function childModal(pid,id){let p=project(pid),x=id&&p.children.find(z=>z.id===id);modal(x?'Редактировать подпроект':'Новый подпроект',`<div class="form-grid">${fld('name','Название',x?.name||'')}${x?'':`<div class="field"><label>Тип подпроекта</label><select name="kind"><option value="construction">Стройка</option><option value="sublease">Субаренда</option></select></div>`}</div>`,f=>{if(x)x.name=f.get('name');else {let kind=f.get('kind');p.children.push(kind==='construction'?{id:uid(),name:f.get('name'),kind:'construction',estimate:[],income:[],expenses:[]}:{id:uid(),name:f.get('name'),kind:'sublease',tenants:[],expenses:[],rentPayments:[]})}})}
function estimateModal(cid,id){let c=child(cid)[1],x=id&&c.estimate.find(z=>z.id===id);modal('Смета',`<div class="form-grid">${fld('name','Позиция',x?.name||'')}${fld('category','Категория',x?.category||'Работы')}${fld('cost','Себестоимость',x?.cost||0,'number')}${fld('client','Цена для клиента',x?.client||0,'number')}</div>`,f=>{let o=x||{id:uid()};Object.assign(o,{name:f.get('name'),category:f.get('category'),cost:+f.get('cost'),client:+f.get('client')});if(!x)c.estimate.push(o)})}
function incomeModal(cid,id){let c=child(cid)[1],x=id&&c.income.find(z=>z.id===id);modal('Приход',`<div class="form-grid">${fld('date','Дата',x?.date||today(),'date')}${fld('amount','Сумма',x?.amount||0,'number')}</div>`,f=>{let o=x||{id:uid()};Object.assign(o,{date:f.get('date'),amount:+f.get('amount')});if(!x)c.income.push(o)})}
...
function delEstimate(c,i){let x=child(c)[1];x.estimate=x.estimate.filter(z=>z.id!==i);save();app()}function delIncome(c,i){let x=child(c)[1];x.income=x.income.filter(z=>z.id!==i);save();app()}function delExpense(t,c,i){let x=expenseOwner(t,c);x.splice(x.findIndex(z=>z.id===i),1);save();app()}function delUtility(c,t,i){let x=child(c)[1].tenants.find(z=>z.id===t);x.utilities=x.utilities.filter(z=>z.id!==i);save();app()}function delRentPayment(c,i){let x=child(c)[1];x.rentPayments=x.rentPayments.filter(z=>z.id!==i);save();app()}
