...
function app(){
  const sidebarProjects=state.projects.map(p=>{
    ...
    return `<div class="sidebar-project ${selected.type==='project'&&selected.id===p.id?'selected-project':''}">
      ...
      <div class="project-meta">${count} ${count===1?'проект':'проекта'}</div>
      <div class="sidebar-children">
        ${p.children.map(c=>{const n=childNet(c);return `<div class="child-row ${selected.type==='child'&&selected.id===c.id?'active':''}" onclick="selectChild('${c.id}')">
          ...
          <button class="side-edit" title="Редактировать проект" onclick="event.stopPropagation();childModal('${p.id}','${c.id}')">✎</button>
        </div>`}).join('')}
      </div>
      <button class="side-btn mini-add" onclick="childModal('${p.id}')">＋ Добавить проект</button>
    </div>`;
  }).join('');
  document.getElementById('app').innerHTML=`<div class="layout"><aside class="sidebar">
    <div class="brand">Финансы проектов</div>
...
function allView(){
  ...
  return `<div class="top"><div><h1>Проекты</h1></div></div>
  ${cards([['Чистая прибыль по всем проектам',total,resultClass(total)]])}
  <div class="panel"><h2>Проекты</h2>
  <div class="projects-list">${state.projects.map(p=>{let n=projectNet(p),count=childCount(p);return `<div class="project-summary" onclick="selectProject('${p.id}')"><div><div class="project-summary-name">📁 ${esc(p.name)}</div><div class="project-summary-meta">${count} ${count===1?'проект':'проекта'}</div></div><div class="project-summary-result ${resultClass(n)}">${signMoney(n)}</div></div>`}).join('')||'<div class="empty">Проекты пока не добавлены</div>'}</div></div>`
}
function sectionView(p){let total=projectNet(p);return `<div class="mobile-back"><button class="btn secondary small" onclick="allSelect()">← Проекты</button></div><div class="top"><div><h1>${esc(p.name)}</h1></div><button class="btn" onclick="childModal('${p.id}')">＋ Добавить проект</button></div>${cards([['Итого по проекту',total,total>=0?'positive':'negative']])}<div class="panel"><h2>Проекты</h2>${p.children.map(c=>{let n=childNet(c);return `<div class="tree-row" style="background:#f7f8fc;margin-bottom:8px" onclick="selectChild('${c.id}')">📂 <b>${esc(c.name)}</b><span class="tree-total ${n>=0?'positive':'negative'}">${signMoney(n)}</span><span class="tree-actions"><button class="iconbtn" title="Редактировать проект" onclick="event.stopPropagation();childModal('${p.id}','${c.id}')">✎</button></span></div>`}).join('')||'<div class="empty">Проектов пока нет</div>'}</div>`}
...
function construction(p,c){
 ...
 return `<div class="mobile-back"><button class="btn secondary small" onclick="selectProject('${p.id}')">← ${esc(p.name)}</button></div><div class="top"><div><h1>${esc(c.name)}</h1><div class="sub">Стройка</div></div><button class="btn secondary" onclick="childModal('${p.id}','${c.id}')">Редактировать</button></div>
...
function sublease(p,c){let totals=subleaseTotals(c),otherExp=totals.expenses,totalIn=totals.income,profit=totals.net;return `<div class="mobile-back"><button class="btn secondary small" onclick="selectProject('${p.id}')">← ${esc(p.name)}</button></div><div class="top"><div><h1>${esc(c.name)}</h1><div class="sub">Субаренда</div></div><button class="btn secondary" onclick="childModal('${p.id}','${c.id}')">Редактировать</button></div>${cards([['Факт. приход',totalIn,'positive'],['Расход',otherExp,'negative'],['Чистая прибыль',profit,profit>=0?'positive':'negative']])}${tabs([['overview','Арендаторы'],['utilities','Коммунальные'],['expenses','Расходы']])}${tab==='overview'?tenantsView(c):tab==='utilities'?utilitiesView(c):subleaseExpenses(c)}`}
...
function childModal(pid,id){let p=project(pid),x=id&&p.children.find(z=>z.id===id);modal(x?'Редактировать проект':'Новый подпроект',`<div class="form-grid">${fld('name','Название',x?.name||'')}${x?'':`<div class="field"><label>Тип проекта</label><select name="kind"><option value="construction">Стройка</option><option value="sublease">Субаренда</option></select></div>`}</div>${x?`<div class="modal-delete"><button type="button" class="btn danger" onclick="deleteChild('${p.id}','${x.id}')">Удалить проект</button></div>`:''}`,f=>{if(x)x.name=f.get('name');else {let kind=f.get('kind');p.children.push(kind==='construction'?{id:uid(),name:f.get('name'),kind:'construction',estimate:[],income:[],expenses:[]}:{id:uid(),name:f.get('name'),kind:'sublease',tenants:[],expenses:[],rentPayments:[]})}})}
...
function deleteProject(id){let p=project(id);if(!p)return;if(confirm(`Удалить проект «${p.name}» вместе со всеми проектами и записями? Это действие нельзя отменить.`)){state.projects=state.projects.filter(x=>x.id!==id);selected={type:'all'};tab='overview';closeModal();save();app()}}
function deleteChild(pid,id){let p=project(pid),c=p?.children.find(x=>x.id===id);if(!p||!c)return;if(confirm(`Удалить проект «${c.name}» вместе со всеми его записями? Это действие нельзя отменить.`)){p.children=p.children.filter(x=>x.id!==id);selected={type:'project',id:p.id};tab='overview';closeModal();save();app()}}
